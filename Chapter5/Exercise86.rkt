...
#reader(lib "htdp-beginner-reader.ss" "lang")((modname Exercise86) (read-case-sensitive #t) (teachpacks ((lib "image.rkt" "teachpack" "2htdp") (lib "universe.rkt" "teachpack" "2htdp"))) (htdp-settings #(#t constructor repeating-decimal #f #t none #f ((lib "image.rkt" "teachpack" "2htdp") (lib "universe.rkt" "teachpack" "2htdp")) #f)))
  ...
  (if (> (string-length ke) 1)
      ...
      (cond
        ...
        [(or (key=? "\t" ke)
             (key=? "\r" ke)) ed]
        [else
         (if (> (string-length (string-append (editor-pre ed)
                                              (editor-post ed)))
                MAX-EDITOR-LENGTH)
             ed
